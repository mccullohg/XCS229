...
class Test_3c(GradedTestCase):
  ...
  def test_2(self):
    ...
    if os.path.exists('./W_sol.txt'):
      ...
      self.assertTrue(W_solution.shape == W_student.shape or W_solution.T.shape == W_student.shape, "shapes of split tensor are not the same")
      self.assertTrue(np.allclose(W_solution, W_student, atol=5e-2) or np.allclose(W_solution.T, W_student, atol=5e-2), "resulting unmixed 5x5 matrix do not match solution")
